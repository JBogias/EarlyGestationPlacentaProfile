...
write_csv(as.data.frame(transGR),
          path = here("data/grch38_txGR.csv.gz"))

write_csv(as.data.frame(genesGR),
          path = here("data/grch38_genesGR.csv.gz"))
...
write_csv(transcript_names,
          here("data/tx_names.csv.gz"))
```

Now load the annotations that have been predefined above
```{r}
gene_anno <- read_csv(here("data/grch38_genesGR.csv.gz"))
tx_anno <- read_csv(here("data/grch38_txGR.csv.gz"))
transcript_names <- read_csv(here("data/tx_names.csv.gz"))
```
...
```{r transCounts}
transCounts <- read_rds(here("data/salmon_object.rds"))
...
```{r load_abundance}
abundance_salmon <- read_csv(here("data/salmon_abundance.csv.gz"))
```
...
```{r autosomeTxpID}
autosomeTxpID <- dplyr::filter(tx_anno, !seqnames %in% c("MT", "X", "Y")) %>%
  dplyr::select(tx_id) %>%
  as.data.frame() %>%
  as.matrix() %>%
  as.character() %>%
...
```{r}
write_csv(oxygenDTU, here("data/oxygenDTU.csv.gz"))
```

The numbers of genes undergoing DTU and transcripts showing a significant change in proportion are shown here
```{r}
sig_dtu <- oxygenDTU %>%
...
### Why are there results for both genes and transcripts?
One might be puzzled by the fact that statistical significance is given for both genes and transcripts. The reason is because this DTU analysis asks two questions of the data:

  1. *Which genes possess transcripts that change in proportions relative to one another, thereby exhibiting differential transcript usage?* And;
  2. *Which transcripts show a significant change in their proportions of overall gene expression, independent of their sister transcripts?*
  
For the first question above, *DRIMSeq* uses a Dirichlet Multinomial model to test for DTU. This will test on a gene-by-gene bases (multiple hypothesis testing). If the null hypothesis for a gene is not retained when corrected for the false discovery rate (FDR < 0.05), then that gene is statistically significant for exhibitng DTU.
For the second question above, a negative binomial model is used to assess for changes in transcript proportions. Does this sound familiar? It works similarly to a differential expression analysis, though we are not provided a log fold change here. Once again, this is tested on a transcript-by-transcript basis (like differential transcript expression). If the null hypothesis for a transcript is not retained after FDR correction (FDR < 0.05), then that transcript is statistically significant for a change in its proportion of its genes overall expression.
...
Here we retrieve proportion percentages in early gestation for transcripts
```{r changing_props}
...
Here we get changing proportions for genes in the DTU analysis
```{r DTU_delta_props}
...
## Global Proportion Changes
Here we visualise the changing proportions of transcripts across the transcriptome. Each transcript is shown with their changing proportions that have either increased of decreased across gestation. As relatively less genes had over 2 distinct transcript isoforms in the filtered dataset, many of these transcripts will exhibit a symmetrical pattern of change. For example, if transcript A has 35% proportion of total gene expression and transcript B has 65%, when one changes, the other must as well.
