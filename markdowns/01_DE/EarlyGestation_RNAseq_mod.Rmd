...
```{r quantile_normalisation_dge, eval=FALSE}
eset <- ExpressionSet(
...
```{r read_distributions_dge, eval=FALSE}
...
```{r quantile_normalisation_dte, eval=FALSE}
eset <- ExpressionSet(
...
```{r read_distributions_dte, eval=FALSE}
qsmooth::qsmoothData(qs) %>%
  as.data.frame() %>%
  rownames_to_column("gene") %>%
  pivot_longer(
